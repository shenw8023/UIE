...
def convert_inputs(tokenizer, prompts: List[str], contents: List[str], max_length=512) -> dict:
    ...
    """
    inputs = tokenizer(text=prompts,                 # ['时间', '地点', '人物']
                        text_pair=contents,          # [sentence, sentence, sentence] 和上面text的数量保持一致
    ...
    for i in range(len(contents)):
        pos_ids += [[j for j in range(len(inputs['input_ids'][i]))]]  # [ ]同一个句子被做多次抽取，可以优化
    pos_ids = torch.tensor(pos_ids)
    inputs['pos_ids']=pos_ids  #[ ]这种绝对位置编码默认可以不提供吧，从后面代码看，这个量压根没被用，试试删了看看
